...
WINDOW_S = 60
GRACE_SECONDS = 30  # Jobs stay valid for window + grace (e.g. 90s for 60s window)
...
def delete_expired_unclaimed_jobs(db, current_window_start: datetime) -> int:
    """Delete only expired unclaimed jobs for *older* windows (window_start < current_window_start).
    Never delete current-window jobs so we don't recreate them in the same window."""
    now = datetime.utcnow()
    deleted = db.query(Job).filter(
        Job.node_id.is_(None),
        Job.expires_at < now,
        Job.window_start < current_window_start,
    ).delete()
    return deleted
...
def run_tick(settings, SessionLocal, server_sk: str) -> tuple[int, int, datetime | None]:
    """
    One scheduler tick: delete expired unclaimed jobs only for older windows;
    ensure current 60s window has K_normal jobs per target (idempotent).
    Job expiry is window-based: expires_at = window_start + window_s + grace.
    """
    now = datetime.utcnow()
    window_start = floor_window_start(now, WINDOW_S)
    issued_at = now
    expires_at = window_start + timedelta(seconds=WINDOW_S + GRACE_SECONDS)

    with SessionLocal() as db:
        deleted = delete_expired_unclaimed_jobs(db, window_start)
        if deleted:
            db.commit()
