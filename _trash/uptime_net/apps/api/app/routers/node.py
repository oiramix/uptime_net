...
from ..models import Job, Node, NodeReputation, PayoutLedger, Receipt
...
def fetch_jobs(
    ...
    # Quarantined nodes must not receive jobs.
...
class ReputationResp(BaseModel):
    node_id: str
    status: str
    agree_count: int
    disagree_count: int
...
class EarningsRecentRow(BaseModel):
    job_id: str
    target_id: str
    window_start: str
    units: int
...
class EarningsResp(BaseModel):
    node_id: str
    total_units: int
    recent: List[EarningsRecentRow]


@router.get("/me/reputation", response_model=ReputationResp)
def get_my_reputation(
    node: Node = Depends(get_current_node),
    db: Session = Depends(get_db),
):
    rep = db.query(NodeReputation).filter(NodeReputation.node_id == node.node_id).first()
    if not rep:
        return ReputationResp(
    ...
    return ReputationResp(
        node_id=node.node_id,
        status=node.status,
        agree_count=rep.agree_count,
        disagree_count=rep.disagree_count,
        quarantined_at=rep.quarantined_at.isoformat() + "Z" if rep.quarantined_at else None,
        updated_at=rep.updated_at.isoformat() + "Z" if rep.updated_at else None,
    )


@router.get("/me/earnings", response_model=EarningsResp)
def get_my_earnings(
    node: Node = Depends(get_current_node),
    db: Session = Depends(get_db),
):
    rows = (
        db.query(PayoutLedger)
        .filter(PayoutLedger.node_id == node.node_id)
        .order_by(PayoutLedger.created_at.desc())
        .limit(50)
        .all()
    )
    total = (
        db.query(func.coalesce(func.sum(PayoutLedger.units), 0))
        .filter(PayoutLedger.node_id == node.node_id)
        .scalar()
    )
    recent = [
        EarningsRecentRow(
            ...
            window_start=r.window_start.isoformat() + "Z",
            units=r.units,
            created_at=r.created_at.isoformat() + "Z",
        )
        for r in rows
    ]
    return EarningsResp(node_id=node.node_id, total_units=int(total), recent=recent)
